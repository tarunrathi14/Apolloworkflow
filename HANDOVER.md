...
> ⚠️ **Read "Open Items" before running anything.** The live sequence is currently **paused**.
...
| Sales Outreach Sequence | **PAUSED** (manual_pause, 2026-08-05) — ~100 contacts frozen mid-flight |
| Contacts in Apollo | 404 total |
| Credits remaining | 2,815 (as of 2026-08-10) |
| Week-4 run (2026-08-10) | **Halted at user request** after filtering; 1 credit spent, nothing saved |

---

## 2. Apollo identifiers

Do not guess these — they are all confirmed from live tool responses.
...
flagged `default: true`. Auto-selecting "the default mailbox" will pick the wrong one. Always pin
the ID explicitly.
...
**Why it changed:** calls were converting at **32% Connected-Positive** while email sat at ~5%
reply with a climbing bounce rate. Email steps were cut 6 → 3 to protect the sending domain.

**Deleted in the restructure:** the LinkedIn message step and two auto-emails. Steps 6 and 8 were
...
**Email (80 delivered):**
- Bounce rate **6.98%** (6 bounced, 3 hard) — ⚠️ above the 5% warning line
- Spam blocked 3 (3.49%)
- Open 7.5% · Reply 6.25% (5 replies) · **1 demo booked** · 1 unsubscribe
...
**Read:** phone is the working channel in this market; email is a support channel and a liability
if fed unverified addresses. ~16% of dialled numbers were bad — check the number before writing
someone off as unresponsive.

**Apollo auto-pause config on this sequence:** warning 5%, auto-pause 8%, min volume 200,
7-day window. At 6.98% the bounce rate is between the two thresholds.
...
sheet `TMT BARS`, column B = client name, column C = qty (MT). 198 clients, period 1 Apr – 28 Jul 2026.
...
| Week 4 | Aug 10 | 3 | — | — | — | 1 (halted) |

**Page rotation:** advance the `page` parameter each week. Pages 1–3 are now mined out.
Week 5 should start at **page 4**.
...
2. **Apollo's `email_status: "verified"` is not always trustworthy.** Check for
   ...
   verified. Three such contacts (54–65% confidence) were enrolled and later stopped.
3. **Search quality degrades sharply by page.** Page 1 was ~85% relevant; page 3 was 35%. The
   `q_organization_keyword_tags` filter pulls in Wipro, Infosys, Bosch, ZETWERK and similar.
4. **Coworking and interior-fitout firms are weak fits** for structural rebar — they buy furniture
   and MEP. Keep them separate from the default enrolment set (WeWork, IndiQube, BHIVE, MOJO
   Campus, Table Space, Livspace, HomeLane, Design Arc Interiors, Zyeta, Simpliwork, Truww).
5. **Karnataka outside Bengaluru is not viable.** Mysore: 39 people total, 6 usable.
   ...
   Apollo's coverage simply isn't there. Don't schedule recurring runs for these.
6. **Apollo tool responses are large.** Most calls overflow the context limit and get written to
...
### 🔴 Blocking
1. **Sequence is paused** (`manual_pause`, 2026-08-05). ~100 contacts frozen. Needs a decision:
   reactivate, or leave off while the bounce rate is addressed. Unknown whether Taarun paused it
   deliberately or in response to the bounce warning — **ask before reactivating**.
2. **`[Name]` placeholder is still in all three call scripts.** Whoever dials needs to be named,
   or reps will read the literal text.

### 🟡 Needs attention
3. **Bounce rate 6.98%** — between Apollo's 5% warning and 8% auto-pause thresholds. The
   restructure reduced future email volume but does not undo the 6 bounces already recorded.
4. **Embassy contacts (3) unresolved.** Chetan Palguna + Alaguraja Arumugam (Embassy Group),
   Mohamad Ahmed (Embassy Services). Client list has "EMBASSY MARBLE & CEMENT COMPANY"
   (199.95 MT) — likely a different entity, but unconfirmed. Left enrolled.
5. **Week-4 candidates were never persisted.** 34 filtered leads (19 builders, 15 fitout) exist
   only in session notes. If the session ends, re-run the page-3 search (1 credit) to recover them.
6. **Draft sequence `6a72b817fe136b0010a6673f` is redundant** now the live one carries the same
   structure. Archive it, or keep as a rollback reference.
7. **Trial-load claim needs verification.** Call scripts and emails offer "twelve tonnes on your
   next slab" for a lab test. Confirm this is an offer President TMT will honour.

### 🟢 Nice to have
8. Tier-2 Karnataka contacts (2 saved, only Rohit Swamy at Marvel Properties is genuinely useful)
   are unenrolled and unworked.
9. Consider whether the weekly cadence should drop below 50 leads given the thinning pool, or
   widen its industry tags.

---

## 9. Weekly workflow definition

The authoritative version lives in the trigger prompt
(`trig_018sFLckhL9PvSv1F3t4e7PY`). Summary:

1. **Search** — `apollo_mixed_people_api_search`, procurement/purchase titles, Bengaluru
   (both person and org location), verified email only, 15+ employees, real-estate/construction
   keyword tags, `per_page: 100`, advancing `page` weekly.
...
3. **Enrich** — `apollo_people_bulk_match` in batches of 10 with `reveal_phone_number: true`,
   then poll `apollo_webhook_result_show` per request_id. Drop anyone whose email is unavailable.
